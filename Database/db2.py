...
def update():
    ...
    messagebox.showinfo("Hi"," Data Updated successfully")
    conn.commit()
    conn.close()
...
def submit():
    ...
    messagebox.showinfo("Hi", "Data Inserted successfully")
    conn.commit()
    conn.close()
...
delete_label.grid(row=12,column=0)
...
query_btn.grid(row=16,column=0,columnspan=2,padx=10,pady=10,ipadx=90)
...
delete_btn.grid(row=13,column=0,columnspan=2,padx=10,pady=10,ipadx=110)
