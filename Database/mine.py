...
my_Image=ImageTk.PhotoImage(Image.open("/Users/ujjwalpariyar/Downloads/italian-foods-concept-menu-design-2528848.jpg"))
...
def submit():
    ...
    messagebox.showinfo("Ujjwal" , "Data Inserted successfully")
    conn.commit()
    conn.close()
#Entry
first_name= Entry(root, width=20,bd=5)
first_name.grid(row=0,column=1)

last_name=Entry(root,width=20,bd=5)
last_name.grid(row=1,column=1)

address=Entry(root,width=20,bd=5)
address.grid(row=2,column=1)

age_num=Entry(root,width=20,bd=5)
age_num.grid(row=3,column=1)

password=Entry(root,width=20,bd=5)
password.grid(row=4,column=1)

father_name=Entry(root,width=20,bd=5)
father_name.grid(row=5,column=1)

city=Entry(root,width=20,bd=5)
city.grid(row=6,column=1)

zipcode=Entry(root,width=20,bd=5)
zipcode.grid(row=7,column=1)

delete_box=Entry(root,width=30)
delete_box.place(x=50,y=340)

#label
l_f_name=Label(root,text="First Name")
l_f_name.grid(row=0,column=0)

l_l_name=Label(root,text="Last Name")
l_l_name.grid(row=1,column=0)

l_address=Label(root,text="Address")
l_address.grid(row=2,column=0)

l_age=Label(root,text="Age")
l_age.grid(row=3,column=0)

l_pass=Label(root,text="Password")
l_pass.grid(row=4,column=0)

l_father_name=Label(root,text="Father Name")
l_father_name.grid(row=5,column=0)

l_city=Label(root,text="City")
l_city.grid(row=6,column=0)

l_zipcode=Label(root,text="Zip code")
l_zipcode.grid(row=7,column=0)

btn=Button(root,text="Add Records",command=submit)
btn.grid(row=11,column=1)
   
query_btn=Button(root, text="Show Records",command=query)
query_btn.grid(row=14,column=0,columnspan=2,padx=10,pady=10,ipadx=110)

edit_btn=Button(root,text="Update",command=edit)
edit_btn.grid(row=17,column=0,columnspan=2,padx=10,pady=10,ipadx=110)

delete_btn=Button(root,text="Delete",command=delete)
delete_btn.grid(row=16,column=0,columnspan=2,padx=10,pady=10,ipadx=110)
